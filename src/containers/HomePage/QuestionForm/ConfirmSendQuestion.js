...
const ConfirmSendQuestion = ({ closeConfirm, create }) => {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="modal-confirm-question-container fixed top-[25%] w-[30%] flex flex-col h-auto bg-white rounded-lg shadow backdrop-blur-md mx-auto mt-16 pb-3 overflow-hidden"
        style={{ left: "35%" }}
      >
        <div className="bg-blue-600 px-3 py-2 w-full">
          <span className="text-white font-semibold">
            {i18n.language === "en"
              ? "Confirm Making Question"
              : "Xác nhận đặt câu hỏi"}
          </span>
        </div>
        <div className="pt-1 pb-4 px-3 flex flex-col justify-center items-center w-full">
          <span>{`${
            i18n.language === "en"
              ? `Are you sure to confirm make this question?`
              : `Bạn có chắc chắn đặt câu hỏi này không?`
          }`}</span>
...
export default ConfirmSendQuestion;
